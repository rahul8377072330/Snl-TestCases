...
public class Board {
    ...
    public Board()
            throws FileNotFoundException, UnsupportedEncodingException,IOException
    		{
        		...
        		BoradModel.init(uuid);
        		data = BoradModel.data(uuid);
    		}
    ...
    public JSONArray registerPlayer(String name) 
            throws PlayerExistsException, GameInProgressExeception,
                FileNotFoundException, UnsupportedEncodingException,
                MaxPlayersReachedException, IOException 
    		{
        		if(data.getJSONArray("players").length()==4){
        		throw new MaxPlayersReachedException(4);
        		}
        		for(Object playerObject:data.getJSONArray("players")){
        			...
        			if(player.getInt("position")!=0){
        				throw new GameInProgressExeception();
        			}
        		}
        		...
        		BoradModel.save(uuid, data);
        		return BoradModel.data(uuid).getJSONArray("players");
    		}
